...
export interface Usuario {
  ...
  nombre: string;
  correo: string;
  /** Rol principal = roles[0]. Se mantiene por compatibilidad; usar `roles` para acceso/UI. */
  rol: Role;
  roles: Role[];
...
interface UsuariosStore {
  ...
  crearUsuario: (data: { nombre: string; correo: string; roles: Role[]; password: string }) => Promise<void>;
  editarUsuario: (
    id: string,
    data: { nombre: string; correo: string; roles: Role[]; password?: string }
  ) => Promise<void>;
...
const SEED_USUARIOS: Usuario[] = [
  { id: "seed-1", nombre: "Admin Principal", correo: "admin@leanstart.com", rol: "administrador", roles: ["administrador"], estado: "activo", creadoEn: "1 ene 2026" },
  { id: "seed-2", nombre: "Juan Pérez", correo: "juan@gmail.com", rol: "emprendedor", roles: ["emprendedor"], estado: "activo", creadoEn: "3 ene 2026" },
  { id: "seed-3", nombre: "Carlos Ruiz", correo: "mentor@gmail.com", rol: "mentor", roles: ["mentor"], estado: "activo", creadoEn: "5 ene 2026" },
  { id: "seed-4", nombre: "Ana López", correo: "evaluador@gmail.com", rol: "evaluador", roles: ["evaluador"], estado: "activo", creadoEn: "8 ene 2026" },
  { id: "seed-5", nombre: "María Fernández", correo: "maria@gmail.com", rol: "emprendedor", roles: ["emprendedor"], estado: "activo", creadoEn: "12 ene 2026" },
  { id: "demo-daniel", nombre: "Daniel", correo: "daniel@example.com", rol: "emprendedor", roles: ["emprendedor", "mentor"], estado: "activo", creadoEn: "7 ago 2026" },
];

function mapUsuario(u: Record<string, unknown>): Usuario {
  const roles = (u.roles as Role[] | undefined)?.length ? (u.roles as Role[]) : [u.rol as Role];
  return {
    id: u.id as string,
  ...
  persist(
    (set, get) => ({
      ...
      async crearUsuario(data) {
        ...
        const nuevo: Usuario = { id, ...resto, rol: resto.roles[0], estado: "activo", creadoEn: ahora };
      ...
      async editarUsuario(id, data) {
        ...
        const { password: _password, ...resto } = data;
        set({
          usuarios: get().usuarios.map((u) => (u.id === id ? { ...u, ...resto, rol: resto.roles[0] } : u)),
        });
      },
