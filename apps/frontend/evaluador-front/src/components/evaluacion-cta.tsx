...
export function EvaluacionCTA({ empresaId }: EvaluacionCTAProps) {
  ...
  return (
    <Link
      ...
      onMouseLeave={(e) => ((e.currentTarget as HTMLElement).style.borderColor = "rgba(154,98,250,0.2)")}
    >
      <div
        className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0"
        style={{ backgroundColor: "rgba(154,98,250,0.14)" }}
      >
        <ClipboardList className="w-4.5 h-4.5" style={{ color: "#9A62FA" }} />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold" style={{ color: "#F2F0F7" }}>Evaluación del Proyecto</p>
      ...
      <span className="inline-flex items-center gap-1 text-xs font-medium shrink-0" style={{ color: "#9A62FA" }}>
