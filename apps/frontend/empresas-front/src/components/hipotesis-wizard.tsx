...
interface HipotesisWizardProps {
  empresaId: string;
  empresaNombre: string;
  /** Cuando se pasa, el wizard continúa una hipótesis ya iniciada en vez de crear una nueva. */
  hipotesisExistente?: Hipotesis;
}

/* ─── Wizard de 3 pasos: crea una hipótesis nueva o continúa una ya iniciada ─── */
export function HipotesisWizard({ empresaId, empresaNombre, hipotesisExistente }: HipotesisWizardProps) {
  ...
  const totalHipotesis = useEmpresasStore((s) => s.empresas.find((e) => e.id === empresaId)?.hipotesisList?.length ?? 0);

  const esNueva = !hipotesisExistente;

  // Al continuar una hipótesis, retoma el paso donde se quedó según la fase
  // guardada (nunca hay un paso 3 "a medias" persistido: el backend solo
  // marca resultados cuando se presiona "Finalizar", momento en que la
  // hipótesis pasa a fase 3 y deja de usar este wizard).
  const pasoInicial: 1 | 2 | 3 = !hipotesisExistente || (hipotesisExistente.fase ?? 1) < 2 ? 1 : 2;

  const [paso, setPaso] = useState<1 | 2 | 3>(pasoInicial);

  // Fase 1
  const [titulo, setTitulo] = useState(hipotesisExistente?.titulo ?? "");
  const [descripcion, setDescripcion] = useState(hipotesisExistente?.descripcion ?? "");
  const [tipoExp, setTipoExp] = useState<TipoExperimento | "">(hipotesisExistente?.experimento?.tipo ?? "");

  // Fase 2
  const [descExp, setDescExp] = useState(hipotesisExistente?.experimento?.descripcion ?? "");
  const [objetivo, setObjetivo] = useState(hipotesisExistente?.experimento?.objetivo ?? "");
  const [criterio, setCriterio] = useState(hipotesisExistente?.experimento?.criterioExito ?? "");
  const [fechaObj, setFechaObj] = useState(hipotesisExistente?.experimento?.fechaObjetivo ?? "");
  ...
  /* ─── Guardado (escribe en store, ya sea creando o continuando) ─── */
  ...
  async function guardarTodo(fase: 1 | 2 | 3, incluirResultados: boolean = fase === 3) {
    ...
    const payload = {
      ...
      estado: "pendiente_validacion" as const,
      fase,
      experimento: {
    ...
    if (hipotesisExistente) {
      await actualizarHipotesis(empresaId, hipotesisExistente.id, payload);
    } else {
  ...
  async function guardarParcial() {
    ...
    try {
      ...
      // llegada de `resultados`, así que aquí nunca se envían aunque el paso 3
      // tenga texto: solo queda en memoria hasta que el usuario finalice).
      await guardarTodo(paso === 1 ? 1 : 2);
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto">
      {/* Back */}
      <Link
        href={`/emprendedor/empresas/${empresaId}`}
      ...
      >
        ...
        <span className="truncate max-w-[180px] md:max-w-none">{empresaNombre}</span>
      </Link>

      <div className="max-w-2xl mx-auto">
        <div className="mb-8">
          <h1 className="text-2xl font-bold" style={{ color: "var(--text-strong)" }}>
            {esNueva ? "Nueva hipótesis" : "Continuar hipótesis"}
          </h1>
          <p className="text-sm mt-1" style={{ color: "var(--text-dim)" }}>
            {esNueva
              ? "Define y valida una suposición de tu modelo de negocio en 3 pasos."
              : "Retoma justo donde lo dejaste para terminar de validarla."}
          </p>
        </div>
