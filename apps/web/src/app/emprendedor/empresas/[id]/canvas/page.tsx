...
function BlockContent({ blockKey, canvas }: { blockKey: BlockKey; canvas: CanvasData }) {
  ...
  if (meta.type === "single") {
    ...
    return (
      <p style={{ color: "rgba(255,255,255,0.82)", fontSize: 11, lineHeight: 1.6, marginTop: 2, borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 8 }}>
  ...
  return (
    <ul style={{ margin: "2px 0 0", padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 4, borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 8 }}>
      {items.map((item, i) => (
        <li key={i} style={{ color: "rgba(255,255,255,0.82)", fontSize: 11, lineHeight: 1.4, display: "flex", gap: 5, alignItems: "flex-start" }}>
                ...
                {(draft as string[]).map((item, i) => (
                  <div key={i} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    <input
                      ...
                      placeholder={openMeta.placeholder}
                      style={{ flex: 1, backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "9px 12px", color: "#F2F0F7", fontSize: 13, outline: "none", fontFamily: "inherit" }}
                      onFocus={(e) => (e.currentTarget.style.borderColor = "rgba(154,98,250,0.45)")}
                      onBlur={(e) => (e.currentTarget.style.borderColor = "rgba(255,255,255,0.08)")}
                    />
